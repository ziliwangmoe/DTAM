...
static void showLenghtOfMat(cuda::GpuMat mat){
	cout << (mat.dataend - mat.datastart) / mat.cols / mat.rows << endl;
}

static cv::Mat saveImgFromGL(float* imgp, string name, int width, int height) {
	cv::Mat img;
	img = cv::Mat(height, width, CV_32FC3, imgp).clone();
	img = img * 255;
...
static void scaleAMat(Mat& mat){
	double maxV, minV;
	minMaxLoc(mat, &minV, &maxV);
	cout << minV << "," << maxV << endl;
	if (mat.type() == CV_8UC1){
		return;
	}
...
static void showColorMat(cuda::GpuMat mat){
	Mat outDept;
	mat.download(outDept);
	imshow("chamo", outDept);
	waitKey(0);
}

static void showMat(cuda::GpuMat mat){
	Mat outDept;
	mat.download(outDept);
	scaleAMat(outDept);
	imshow("chamo", outDept);
	outDept = outDept * 255;
	outDept.convertTo(outDept, CV_8UC1);
	imwrite("chamo.jpg", outDept);
	waitKey(0);
}

static void dumpToFile(Mat mat, string name){
	FileStorage fs("chamo.xml", FileStorage::APPEND);
	fs << name << mat;
	fs.release();
}

static void dumpToFileGPU(cuda::GpuMat mat){
	FileStorage fs("chamo.xml", FileStorage::WRITE);
	Mat outDept;
	mat.download(outDept);
	//scaleAMat(outDept);
	fs << "chamo"<<outDept;
